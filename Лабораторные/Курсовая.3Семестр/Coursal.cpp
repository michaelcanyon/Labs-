...
*File Name : Coursal.cpp									      *
*Language : С                                                     *
*Programer: Чечуга Михаил                                         *
*М3О-224Бк-17                                                     *
*Created : 25/11/18                                               *
*LastRevision: 26/12/18                                           *
******************************************************************/
...
int main()
{
	...
	char *filename = "CoursalFile.txt";
	char *sin = "wordssin.txt";
	char *copy = "copy1.txt";
	char *outfilename = "CoursaloutFile.txt";
	...
	int ch = 0;
	int nk = 0;
	//файл на чтение слов и синонимов
	...
	while (fgets(buffer, 256, fin) != NULL)
	{
		...
		while (ch != 1)
		{
			//читает синоним и слово и проверяет, корректно ли они считаны
			...
			//приведение синонима к регистру исходного слова
			int wordlength = 0;
...
void ReplaceWords(char *&string, char *OldWord, char *NewWord, char *newBuffer)
{
	...
	while ((i = strstr(i, OldWord)) != NULL)
	{
		...
		{
			wordcheck -= 1;
			if (!(*wordcheck == '.' || *wordcheck == ',' || *wordcheck == ' ' || *wordcheck == ';' || *wordcheck == '\n' || *wordcheck == '<'
				|| *wordcheck == '>' || *wordcheck == ',' || *wordcheck == ':' || *wordcheck == '"' || *wordcheck == '?' || *wordcheck == '!'
		...
		if (!(*wordcheck == '.' || *wordcheck == ',' || *wordcheck == ' ' || *wordcheck == ';' || *wordcheck == '\n' || *wordcheck == '<'
			|| *wordcheck == '>' || *wordcheck == ',' || *wordcheck == ':' || *wordcheck == '"' || *wordcheck == '?' || *wordcheck == '!'
			|| *wordcheck == ')' || *wordcheck == '(' || *wordcheck == '-' || *wordcheck == '\0'))
		{
			i += strlen(OldWord);
			check = 1;
			continue;
...
//Считывает заменяемое слово или его синоним из файла
int getWordOrSynonim(char *array, FILE *stream)
{
	array[0] = '\0';
	...
	{
		//Добавил условие isspace. В файле пары слово-синоним было неудобно читать. Чтение продолжается, если встретился \n
